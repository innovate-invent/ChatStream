...
import discordChat from "./discord.js";
...
export async function start() {
    ...
    if (accessToken) {
        source = await twitchChat(accessToken);
        addSource(source);
    }
...
export default async function twitchChat(accessToken: string) {
    ...
    async function handleMessage(message: any,
                                 ...
                                 registerHeartbeat: (timeout: number) => void,
        ...
        switch (message.metadata.message_type) {
            case 'session_welcome':
                ...
                keepaliveTimeout = (message as WelcomeMessage).payload.session.keepalive_timeout_seconds * 1000;
                registerHeartbeat(keepaliveTimeout);
                break;
                ...
                switch ((message as NotificationMessage).payload.subscription.type) {
                    case 'channel.chat.message':
                        ...
                        pushMessage({
                            ...
                            //userBadgeUrl:
                            userName: messageEvent.chatter_user_name,
