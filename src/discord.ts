...
export async function start() {
    ...
    if (accessToken && channelId) {
        source = await discordChat(accessToken, channelId);
        addSource(source);
    }
...
export default async function discordChat(accessToken: string, channelId: string): Promise<AsyncGenerator<StreamMessage, void, unknown>> {
            ...
            case GatewayOpcodes.Dispatch:
                switch (payload.t) {
                    case "READY":
                        ...
                        console.info("Discord: Received Ready", sessionId);
                        break;
                    case "RESUMED":
                        console.info("Discord: Received Resumed");
                        break;
                    case "MESSAGE_CREATE": // fall-through
                    case "MESSAGE_UPDATE":
